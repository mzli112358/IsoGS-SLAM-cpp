...
def find_latest_checkpoint(results_dir):
    """
    自动查找最新的检查点文件，并返回路径和实际训练帧数。
    优先使用 params.npz（最终文件），如果不存在则查找最新的 params{num}.npz
    
    返回: (checkpoint_path, trained_frames)
        - checkpoint_path: 检查点文件路径
        - trained_frames: 实际训练帧数（从文件名提取，如果无法提取则返回 None）
    """
    ...
    if os.path.exists(params_npz_path):
        ...
        # params.npz 是最终文件，无法从文件名提取帧数，返回 None
    ...
    if checkpoint_files:
        ...
        trained_frames = latest_checkpoint[0]  # 从文件名提取的实际训练帧数
        print(f"✓ 自动选择最新检查点: {latest_checkpoint[1]} (帧 {trained_frames})")
        if len(checkpoint_files) > 1:
            print(f"  (共找到 {len(checkpoint_files)} 个检查点)")
        return checkpoint_path, trained_frames
    else:
...
if __name__=="__main__":
    ...
    # 确定场景参数文件路径和实际训练帧数
    trained_frames = None  # 从检查点文件名提取的实际训练帧数
    if 'scene_path' in config:
        ...
        match = pattern.match(filename)
        if match:
            trained_frames = int(match.group(1))
            print(f"从检查点文件名提取的训练帧数: {trained_frames}")
    else:
        # 如果配置中没有指定 scene_path，自动查找最新检查点
        print("配置中未指定 scene_path，自动查找最新检查点...")
        scene_path, trained_frames = find_latest_checkpoint(results_dir)
    ...
    # 获取实际训练帧数
    # 优先使用从文件名提取的帧数，如果无法提取则使用 cam_unnorm_rots 的维度
    if trained_frames is not None:
        checkpoint_num_frames = trained_frames
        # 训练帧数已在前面打印过，这里不再重复
    else:
        # 如果无法从文件名提取（如 params.npz），使用 cam_unnorm_rots 的维度
        # 注意：这可能不准确，因为 cam_unnorm_rots 是按数据集总帧数初始化的
        checkpoint_num_frames = params['cam_unnorm_rots'].shape[-1]
        print(f"警告: 无法从文件名提取训练帧数，使用 cam_unnorm_rots 维度: {checkpoint_num_frames}")
        print(f"注意: 这可能不准确，因为 cam_unnorm_rots 是按数据集总帧数初始化的")
    ...
    if dataset_config['use_train_split']:
        ...
        if num_frames > checkpoint_num_frames:
            print(f"注意: 数据集有 {num_frames} 帧，但只评估前 {eval_num_frames} 帧（训练时见过的帧）")
    else:
        # 新视角合成评估：评估所有帧，用于测试模型泛化能力
        ...
        if num_frames > checkpoint_num_frames:
            ...
            print(f"将评估所有 {eval_num_frames} 帧以测试模型泛化能力")
